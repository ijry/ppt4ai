...
const runProps = '<a:rPr><a:solidFill><a:srgbClr val="FF0000"/></a:solidFill><a:highlight><a:srgbClr val="FFFF00"/></a:highlight></a:rPr>'

function sourcePackage(rPr = runProps): Uint8Array {
...
describe('text highlight survives writeback', () => {
  it('imports the highlight colour', async () => {
    ...
    expect(element.body?.paragraphs[0]?.runs[0]?.marks?.highlight).toEqual({ type: 'srgb', v: 'FFFF00' })
  })
  ...
  it('writes a:highlight after the fill when a text edit forces a rewrite', async () => {
    const source = sourcePackage()
    ...
    expect(xml).toContain('<a:solidFill><a:srgbClr val="FF0000"/></a:solidFill><a:highlight><a:srgbClr val="FFFF00"/></a:highlight>')
