...
export interface PlaygroundAssetHost {
  ...
  addMaster(sourceMasterId: string): PlaygroundAssetHostSnapshot
...
export function createPlaygroundAssetHost(options: PlaygroundAssetHostOptions = {}): PlaygroundAssetHost {
  ...
  return {
    ...
    addLayout(sourceLayoutId) {
      if (!engine.getState().document.layouts?.[sourceLayoutId]) return fail('layout-missing')
      try {
        engine.dispatch({ type: 'addLayout', sourceLayoutId })
    ...
    addMaster(sourceMasterId) {
      if (!engine.getState().document.masters?.[sourceMasterId]) return fail('master-missing')
      try {
        engine.dispatch({ type: 'addMaster', sourceMasterId })
        status = { kind: 'success', message: 'master-added' }
      } catch {
        return fail('master-add-failed')
      }
      return snapshot()
    },
