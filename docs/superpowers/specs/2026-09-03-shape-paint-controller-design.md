...
> 状态：**已废弃并删除**（2026-09-03，实现 `9fd7f28`，删除见下）
> 日期：2026-09-03

> **废弃原因（同日）**：下一刀把 paint 工具栏从 playground 侧栏移进 `PptEditor`（`7660131`），
> 而 `PptEditor` 是表现型组件、**没有 engine**，所以它无法使用这个控制器 —— 它直接从
> `scene` 与 `selectedElementIds` 算展示模型并 emit 意图。playground 也不用控制器，它走
> 既有的 `asset-host` 方法。于是控制器成了**与 `PptEditor` 重复的死代码**，正是本设计
> 决策 1 想避免的「两处会漂移」。已连同其测试删除；本文件保留，记录当时的推理与它为何
> 被取代。决策 1（展示值取自场景而非元素）与决策 4（多选与不可承载 kind 禁用）**都活了
> 下来**，只是落在 `PptEditor` 里而不是控制器里。
