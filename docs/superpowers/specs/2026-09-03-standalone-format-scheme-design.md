...
**补齐的条目带默认线宽，已建模的条目缺 `w` 就不写 `w`** —— 后者是「缺 `w` 时不发明默认宽度」那条纪律的延续：省略 `w` 在 OOXML 里意味着继承，凭空补一个会让指向它的每个形状都错。补齐的条目本来整条都是发明的，给它 Office 的宽度不额外增加谎言。
