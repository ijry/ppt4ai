...
**`hue` 与 `hueOff` 不做数学**：它们在 schema 里是**角度**（1/60000 度）而不是百分比，与 `hueMod` 单位不同 —— 这一点我无法从手头资料核实，猜错会把色相转到别处。两者仍**逐字保存**（决策 1），只是不参与计算。`gamma`/`invGamma` 与逐通道的 `red*`/`green*`/`blue*` 同样只保存不计算。

**无 `val` 的变换仍然丢**：`a:comp`/`a:inv`/`a:gray` 是开关式元素、没有 `val` 属性，而 `ColorTransform.value` 是必填数值，因此它们进不了模型（实测：探针里的 `<a:comp/>` 不出现在导入结果中）。要收下它们需要给 `ColorTransform` 加一个无值形态，与本刀的「放宽类型与范围」不是同一件事，延期并写进限制。
...
- `hue`/`hueOff`（角度单位未核实）、`gamma`/`invGamma`、逐通道 `red*`/`green*`/`blue*` 只保存不计算：颜色按未施加它们的样子渲染
- `a:comp`/`a:inv`/`a:gray` 没有 `val`，因此仍不进模型、仍会在无源导出时丢失（见决策 2 末段）
