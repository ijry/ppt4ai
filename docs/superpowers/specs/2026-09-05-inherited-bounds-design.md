...
**改 bounds 写不回去**——**已在 `6cd4937` 修掉**。本刀当时把它记为已知限制，理由是「今天这类元素根本不存在，更谈不上移动」；那条理由随本刀本身失效（元素变得可见可拖动，而拖动静默丢失），因此下一刀立即补上：写回在源包没有 `a:xfrm` 且模型 bounds 与继承值不同时按 ECMA 序列插入该节点。见 [2026-09-05-insert-xfrm-design.md](./2026-09-05-insert-xfrm-design.md)。

## 8. 实现记录（2026-09-05）

实现提交 `08525c5`。**第 3 节的第一条理由是错的，测试当场抓住**，因此本刀比设计大一圈：
