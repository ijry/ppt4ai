...
实现提交 `cfa030a`。按设计执行，两处与设计不同：

**写回起初留了缺口，随后补上**。本刀实现时只依赖 `lineJoinReplacements`「转角本身变化才重写元素」的既有行为：模型持有源包的 `lim`、两边一致，因此无关编辑不触碰它，但**单改 `lim` 而不改转角类型写不出去**。当时判断「没有 UI 能产生这种编辑」而记为已知缺口——这条判断不完整：程序直接驱动模型正是本项目的用途。缺口已在 `line-vocabulary-writeback.test.ts` 那刀补上，做法是把 `lim` 折进 `lineJoinReplacements` 由同一个函数负责整个元素（两个补丁若各自处理同一段范围会冲突），比较落在值上而非序列化结果上，因此源包换个写法拼出同样的限制不会被重写。
