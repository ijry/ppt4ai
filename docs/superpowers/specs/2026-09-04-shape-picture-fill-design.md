...
> 状态：已实现（2026-09-04，`e7f42a4`、`65c65bf`、`580aafe`、`4e11eb6`、`062ae63`、`c2a6315`）
> 日期：2026-09-04
...
**没有 fill mode 的 `a:blipFill` 读作 stretch**：既然没有重复要摆，拉伸是唯一可表达的读法，也正是 `p:pic` 一直对自己的 blip 做的事（它根本不看 fill mode）。
...
- 工具栏对图片填充形状显示的是空填充色（`PptEditor.vue` 从场景的已解析颜色取值，而图片填充刻意没有）
- **未在跑起来的应用里目视确认**：playground 种子文档由代码构造、没有图片填充形状，要看见得先往 demo 里加内容
