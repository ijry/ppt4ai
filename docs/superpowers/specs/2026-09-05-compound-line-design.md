...
实现提交 `0627adc`。按设计执行，两处值得记：

**`lineCapReplacements` 抽成了通用的 `lineAttributeReplacements`**。设计说「照 `cap` 抄两份」，实现时发现三份逐字相同、只有属性名不同，于是抽成一个带 `name` 参数的函数，三个调用各一行。抽的过程中把字面正则改成 `new RegExp`，**转义写错了一次**（模板字符串里要写 `\\s` 才能得到正则的 `\s`，只写一个反斜杠得到的是「反斜杠加 s」这个字面量而非空白符），于是属性永远匹配不上、删不掉——`stroke-cap-join.test.ts` 立刻标红。既有测试在这里救了一次：如果 `cap` 没有「删除属性」那条测试，这个 bug 会静默留下。

**`theme-line-style.test.ts` 的一条预期要更新**：它的 fixture 第一个条目一直写着 `cmpd="sng" algn="ctr"`，现在读得出来了，所以记录里多两个字段。与 `cap`/`join` 那刀遇到 `cap="flat"` 是同一回事——fixture 早就带着这些属性，只是此前没人读。
