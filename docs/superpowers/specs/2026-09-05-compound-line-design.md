...
**`lineCapReplacements` 抽成了通用的 `lineAttributeReplacements`**。设计说「照 `cap` 抄两份」，实现时发现三份逐字相同、只有属性名不同，于是抽成一个带 `name` 参数的函数，三个调用各一行。抽的过程中把字面正则改成 `new RegExp`，**转义写错了一次**（`\s` 写成 `\s`，在 TS 源码里是「反斜杠加 s」而非空白符），于是属性永远匹配不上、删不掉——`stroke-cap-join.test.ts` 立刻标红。既有测试在这里救了一次：如果 `cap` 没有「删除属性」那条测试，这个 bug 会静默留下。
