...
**描边的填充节点同样已改为补丁**（`3e2b3f3` 之后紧接的一刀，同一个 `sameKindFillPatches`）：`a:ln` 里的 `a:solidFill`/`a:gradFill` 原先也是整块替换。共用时多做了一件事——片段按填充节点自己的命名空间前缀重写，因为 `a:ln` 可能带别的前缀，这也正是 `serializeFillForLine` 存在的理由。
