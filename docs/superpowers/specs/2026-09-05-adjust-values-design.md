...
源包写回本刀**不改**：它已经原样保留，且模型持有的值与源一致，因此无关编辑不会重写。**但这不等于可编辑**——单改可调值写不出去，因为 `geometryReplacements` 只比较 `prst` 词。该缺口已在后续一刀补上（`geometryReplacements` 增加 `a:avLst` 比较，词与列表各自生成替换，同时改两者也不冲突）。
...
**模型文件里的注释不能包含 `*/`**。设计稿在 `AdjustValue` 的文档注释里举例写了公式语言的 `*/ a b c` 形式，那两个字符提前闭合了块注释，`tsc` 报了一串 `TS1434`/`TS1443`。改成用文字描述（「multiply-divide」）。这条对整个仓库成立——凡是要在注释里提 OOXML 公式语言的地方都会踩。
