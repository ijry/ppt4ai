...
- **实现期由测试逼出的一处修正**：`emuFromPoints('')` 原本返回 `0`（`Number('')` 是 `0`），于是「清空输入框」会被当成「设显式发丝线」。组件里本有 `raw === ''` 的守卫，但辅助函数自己也必须诚实 —— 否则下一个调用点忘了守卫就会静默设 0。已改为返回 `undefined` 并加断言。
- **另一处**：测试第一版用 `Partial<Props>` 展开传 props，`exactOptionalPropertyTypes` 下显式 `undefined` 的可选 prop 是类型错误（vitest 能跑、`vue-tsc` 报错）。改为逐项条件展开。
