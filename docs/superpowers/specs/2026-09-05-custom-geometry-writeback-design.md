...
实现提交 `5bf6c71`。按设计执行，无偏离。

**决策 3 的那条一致性测试立刻证明了自己值得写**：它有两个版本（基础路径与含 `arcTo`/`quadBezTo` 的路径），两者都一次通过——但写它的过程中我必须逐个核对镜像与 `parseCustomGeometry` 的判据（点数要求、`arcTo` 的四个属性缺一即整条路径作废、`commands` 为空的路径跳过、`w`/`h` 缺席时字段缺席）。**没有这条测试，这些判据的任何一处不一致都只会表现为「某些文件被无谓重写」或「某些编辑被吞」，而两者都不会有人注意到。**

区分力已验证：删掉 `geometryReplacements` 里那段路径比较，八条里一条标红（`writes a changed path list`）。只有一条是对的——其余七条钉的是「不该动的东西没被动」，而删掉新代码正是让它们全部保持通过的做法。
